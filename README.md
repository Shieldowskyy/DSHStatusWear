...
Alternatywnie: **Build → Build Bundle(s) / APK(s) → Build APK(s)** (lub zmień wariant na `release` w karcie *Build Variants*), znajdziesz zoptymalizowany plik `.apk` w `app/build/outputs/apk/release/yt.dsh.statuswear-wearos-v1.0-universal-release.apk` (o małym rozmiarze ~3–5 MB) i zainstalujesz go ręcznie przez `adb install`.
